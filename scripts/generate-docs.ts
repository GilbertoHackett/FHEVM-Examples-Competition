 ...
 *
 * Example: ts-node scripts/generate-docs.ts privacy-procurement --output docs/
 */
...
const EXAMPLES_CONFIG: Record<string, DocsConfig> = {
  'privacy-procurement': {
    title: 'Privacy-Preserving Procurement',
    description: 'This example demonstrates how to build a confidential procurement system using FHEVM for private bidding and vendor selection.',
    contract: 'contracts/PrivacyProcurement.sol',
    test: 'test/Counter.test.ts',
    output: 'docs/privacy-procurement.md',
...
function main(): void {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    ...
    log('  ts-node scripts/generate-docs.ts privacy-procurement');
